...
var project1 = function(p) {
    ...
    p.setup = function () {
        ...
        p.createP("Arrow keys scroll. Clicking changes tiles.").parent("container1");
    ...
    */
    ...
    let [tw, th] = [p.p3_tileWidth(), p.p3_tileHeight()];
    ...
    p.drawBranch = function (x, y, angle, length, thickness, branchColor, leafColor) {
    ...
    p.drawLeaves = function (x, y, angle, length, color) {
...
var project2 = function(q) {
    ...
    // These are actually slightly weirder than in full 3d...
    /////////////////////////////
    q.worldToScreen = function ([world_x, world_y], [camera_x, camera_y]) {
        let i = (world_x - world_y) * tile_width_step_main;
        let j = (world_x + world_y) * tile_height_step_main;
        return [i + camera_x, j + camera_y];
    }

    q.worldToCamera = function ([world_x, world_y], [camera_x, camera_y]) {
        let i = (world_x - world_y) * tile_width_step_main;
        let j = (world_x + world_y) * tile_height_step_main;
        return [i, j];
    }

    q.tileRenderingOrder = function (offset) {
        return [offset[1] - offset[0], offset[0] + offset[1]];
    }

    q.screenToWorld = function ([screen_x, screen_y], [camera_x, camera_y]) {
        screen_x -= camera_x;
        screen_y -= camera_y;
        screen_x /= tile_width_step_main * 2;
        screen_y /= tile_height_step_main * 2;
    ...
    q.cameraToWorldOffset = function ([camera_x, camera_y]) {
        let world_x = camera_x / (tile_width_step_main * 2);
        let world_y = camera_y / (tile_height_step_main * 2);
        return { x: Math.round(world_x), y: Math.round(world_y) };
    }

    q.worldOffsetToCamera = function ([world_x, world_y]) {
        let camera_x = world_x * (tile_width_step_main * 2);
        let camera_y = world_y * (tile_height_step_main * 2);
        return new p5.Vector(camera_x, camera_y);
    }
    ...
    q.setup = function () {
        let canvas = q.createCanvas(800, 400);
        canvas.parent("container2");

        camera_offset = new p5.Vector(-q.width / 2, q.height / 2);
        camera_velocity = new p5.Vector(0, 0);
        ...
        let input = q.createInput("xyzzy");
        input.parent(label);
        input.input(() => {
            q.rebuildWorld(input.value());
        });

        q.createP("Arrow keys scroll. Clicking changes tiles.").parent("container2");

        q.rebuildWorld(input.value());
    }

    q.rebuildWorld = function (key) {
        if (window.p3_worldKeyChanged) {
            window.p3_worldKeyChanged(key);
        }
        tile_width_step_main = window.p3_tileWidth ? window.p3_tileWidth() : 32;
        tile_height_step_main = window.p3_tileHeight ? window.p3_tileHeight() : 14.5;
        tile_columns = Math.ceil(q.width / (tile_width_step_main * 2));
        tile_rows = Math.ceil(q.height / (tile_height_step_main * 2));
    }

    q.mouseClicked = function () {
        let world_pos = q.screenToWorld(
            [0 - q.mouseX, q.mouseY],
            [camera_offset.x, camera_offset.y]
        );
    ...
    q.draw = function () {
        // Keyboard controls!
        if (q.keyIsDown(q.LEFT_ARROW)) {
            camera_velocity.x -= 1;
        }
        if (q.keyIsDown(q.RIGHT_ARROW)) {
            camera_velocity.x += 1;
        }
        if (q.keyIsDown(q.DOWN_ARROW)) {
            camera_velocity.y -= 1;
        }
        if (q.keyIsDown(q.UP_ARROW)) {
            camera_velocity.y += 1;
        }
        ...
        let world_offset = q.cameraToWorldOffset([camera_offset.x, camera_offset.y]);
        ...
        for (let y = y0; y < y1; y++) {
            for (let x = x0; x < x1; x++) {
                q.drawTile(q.tileRenderingOrder([x + world_offset.x, y - world_offset.y]), [
                    camera_offset.x,
                    camera_offset.y
                ]); // odd row
            }
        ...
        q.describeMouseTile(world_pos, [camera_offset.x, camera_offset.y]);
    ...
    // Display a discription of the tile at world_x, world_y.
    q.describeMouseTile = function ([world_x, world_y], [camera_x, camera_y]) {
        let [screen_x, screen_y] = q.worldToScreen(
            [world_x, world_y],
            [camera_x, camera_y]
        );
        q.drawTileDescription([world_x, world_y], [0 - screen_x, screen_y]);
    }

    q.drawTileDescription = function ([world_x, world_y], [screen_x, screen_y]) {
        q.push();
        q.translate(screen_x, screen_y);
        if (window.p3_drawSelectedTile) {
            window.p3_drawSelectedTile(world_x, world_y, screen_x, screen_y);
        }
        q.pop();
    ...
    // Draw a tile, mostly by calling the user's drawing code.
    q.drawTile = function ([world_x, world_y], [camera_x, camera_y]) {
        let [screen_x, screen_y] = q.worldToScreen(
            [world_x, world_y],
            [camera_x, camera_y]
        );
        q.push();
        q.translate(0 - screen_x, screen_y);
        if (window.p3_drawTile) {
            window.p3_drawTile(world_x, world_y, -screen_x, screen_y);
        }
        q.pop();
    ...
    */
    ...
    q.p3_worldKeyChanged = function (key) {
        worldSeed = XXH.h32(key, 0);
        noiseSeed(worldSeed);
        randomSeed(worldSeed);
    }
    ...
    //dimensions of tiles
    q.p3_tileWidth = function () {
        return 32;
    }
    q.p3_tileHeight = function () {
        return 16;
    }

    let [tw, th] = [q.p3_tileWidth(), q.p3_tileHeight()];

    let clicks = {};

    q.p3_tileClicked = function (i, j) {
        let key = [i, j];
        clicks[key] = 1 + (clicks[key] | 0);
    }

    q.p3_drawBefore = function () { }

    q.p3_drawTile = function (i, j) {
        noStroke();
        let isGreenTile = false;
        if (XXH.h32("tile:" + [i, j], worldSeed) % 4 == 0 || XXH.h32("tile:" + [i, j], worldSeed) % 3 == 0 || XXH.h32("tile:" + [i, j], worldSeed) % 2 == 0) {
            //ONE EXAMPLE OF TILE    
        ...
        if (drawTree && isGreenTile) {
            ...
            drawBranch(0, th / 4, PI / 2, 20, 8, branchColor, "Orange"); // Branches start from the top of the trunk
    ...
    q.p3_drawSelectedTile = function (i, j) {
        noFill();
        stroke(0, 255, 0, 128);
    ...
    q.p3_drawAfter = function () { }

    q.drawBranch = function (x, y, angle, length, thickness, branchColor, leafColor) {
        // Draw the main branch
        stroke(branchColor);
        strokeWeight(thickness);
        line(x, y, x + cos(angle) * length, y - sin(angle) * length);
    ...
    q.drawLeaves = function (x, y, angle, length, color) {
        let numLeaves = 5; // Number of leaves
        let leafSize = 20; // Size of each leaf
        let leafInterval = length / (numLeaves + 1); // Interval between leaves
...
var project3 = function(r) {
    ...
    // These are actually slightly weirder than in full 3d...
    /////////////////////////////
    r.worldToScreen = function ([world_x, world_y], [camera_x, camera_y]) {
        let i = (world_x - world_y) * tile_width_step_main;
        let j = (world_x + world_y) * tile_height_step_main;
        return [i + camera_x, j + camera_y];
    }

    r.worldToCamera = function ([world_x, world_y], [camera_x, camera_y]) {
        let i = (world_x - world_y) * tile_width_step_main;
        let j = (world_x + world_y) * tile_height_step_main;
        return [i, j];
    }

    r.tileRenderingOrder = function (offset) {
        return [offset[1] - offset[0], offset[0] + offset[1]];
    }

    r.screenToWorld = function ([screen_x, screen_y], [camera_x, camera_y]) {
        screen_x -= camera_x;
        screen_y -= camera_y;
        screen_x /= tile_width_step_main * 2;
        screen_y /= tile_height_step_main * 2;
    ...
    r.cameraToWorldOffset = function ([camera_x, camera_y]) {
        let world_x = camera_x / (tile_width_step_main * 2);
        let world_y = camera_y / (tile_height_step_main * 2);
        return { x: Math.round(world_x), y: Math.round(world_y) };
    }

    r.worldOffsetToCamera = function ([world_x, world_y]) {
        let camera_x = world_x * (tile_width_step_main * 2);
        let camera_y = world_y * (tile_height_step_main * 2);
        return new p5.Vector(camera_x, camera_y);
    }
    ...
    r.setup = function () {
        let canvas = r.createCanvas(800, 400);
        canvas.parent("container3");

        camera_offset = new p5.Vector(-r.width / 2, r.height / 2);
        camera_velocity = new p5.Vector(0, 0);
        ...
        let input = r.createInput("xyzzy");
        input.parent(label);
        input.input(() => {
            r.rebuildWorld(input.value());
        });

        r.createP("Arrow keys scroll. Clicking changes tiles.").parent("container3");

        r.rebuildWorld(input.value());
    }

    r.rebuildWorld = function (key) {
        if (window.p3_worldKeyChanged) {
            window.p3_worldKeyChanged(key);
        }
        tile_width_step_main = window.p3_tileWidth ? window.p3_tileWidth() : 32;
        tile_height_step_main = window.p3_tileHeight ? window.p3_tileHeight() : 14.5;
        tile_columns = Math.ceil(r.width / (tile_width_step_main * 2));
        tile_rows = Math.ceil(r.height / (tile_height_step_main * 2));
    }

    r.mouseClicked = function () {
        let world_pos = r.screenToWorld(
            [0 - r.mouseX, r.mouseY],
            [camera_offset.x, camera_offset.y]
        );
    ...
    r.draw = function () {
        // Keyboard controls!
        if (r.keyIsDown(r.LEFT_ARROW)) {
            camera_velocity.x -= 1;
        }
        if (r.keyIsDown(r.RIGHT_ARROW)) {
            camera_velocity.x += 1;
        }
        if (r.keyIsDown(r.DOWN_ARROW)) {
            camera_velocity.y -= 1;
        }
        if (r.keyIsDown(r.UP_ARROW)) {
            camera_velocity.y += 1;
        }
        ...
        let world_offset = r.cameraToWorldOffset([camera_offset.x, camera_offset.y]);
        ...
        for (let y = y0; y < y1; y++) {
            for (let x = x0; x < x1; x++) {
                r.drawTile(r.tileRenderingOrder([x + world_offset.x, y - world_offset.y]), [
                    camera_offset.x,
                    camera_offset.y
                ]); // odd row
            }
        ...
        r.describeMouseTile(world_pos, [camera_offset.x, camera_offset.y]);
    ...
    // Display a discription of the tile at world_x, world_y.
    r.describeMouseTile = function ([world_x, world_y], [camera_x, camera_y]) {
        let [screen_x, screen_y] = r.worldToScreen(
            [world_x, world_y],
            [camera_x, camera_y]
        );
        r.drawTileDescription([world_x, world_y], [0 - screen_x, screen_y]);
    }

    r.drawTileDescription = function ([world_x, world_y], [screen_x, screen_y]) {
        r.push();
        r.translate(screen_x, screen_y);
        if (window.p3_drawSelectedTile) {
            window.p3_drawSelectedTile(world_x, world_y, screen_x, screen_y);
        }
        r.pop();
    ...
    // Draw a tile, mostly by calling the user's drawing code.
    r.drawTile = function ([world_x, world_y], [camera_x, camera_y]) {
        let [screen_x, screen_y] = r.worldToScreen(
            [world_x, world_y],
            [camera_x, camera_y]
        );
        r.push();
        r.translate(0 - screen_x, screen_y);
        if (window.p3_drawTile) {
            window.p3_drawTile(world_x, world_y, -screen_x, screen_y);
        }
        r.pop();
    ...
    */
    ...
    r.p3_worldKeyChanged = function (key) {
        worldSeed = XXH.h32(key, 0);
        noiseSeed(worldSeed);
        randomSeed(worldSeed);
    }
    ...
    //dimensions of tiles
    r.p3_tileWidth = function () {
        return 32;
    }
    r.p3_tileHeight = function () {
        return 16;
    }

    let [tw, th] = [r.p3_tileWidth(), r.p3_tileHeight()];

    let clicks = {};

    r.p3_tileClicked = function (i, j) {
    ...
    r.p3_drawBefore = function () { }

    r.p3_drawTile = function (i, j) {
        noStroke();
        let isGreenTile = false;
        if (
    ...
    r.p3_drawSelectedTile = function (i, j) {
        noFill();
        stroke(0, 255, 0, 128);
    ...
    r.p3_drawAfter = function () { }

    r.drawMountain = function (x, y) {
...
let sketch1 = new p5(project1, 'p1');
let sketch2 = new p5(project2, 'p2');
let sketch3 = new p5(project3, 'p3');
